...
- **`electrical.*` is green.** `plan/mep.py` has to carry ED-RADON-FAN-JB (MN Rules
  1303.2402 subpart 6), and `electrical.room_lighting` / `electrical.receptacle_spacing`
  both gate on "does this plan model electrical at all" — so that mandatory box would flip
  them from *not modeled* to *modeled and incomplete* without more. `plan/electrical.py` +
  `plan/circuits.py` are the smallest honest answer: one panel, four circuits, a light +
  switch in each habitable room, and NEC 210.52 receptacles on their wall lines. Unlike a
  rim-band flashing detail, none of that is an opinion about a specific building.
