...
| `court_soakaway_storage.md` | `checks/mep/soakaway_storage.py` (`drainage.soakaway_storage`) — the court's 12" soakaway course (388 sf, 155 cf of voids + 93 cf of 48 h presumed infiltration) against 222 cf of design snowmelt off `AD-SG-COURT`'s catchment, and the relief lip inside the drained section (`tests/test_catlin_court_drainage.py`); §7 is the design basis for the pipeless W1→W2 lateral through the stone | live |
| `rain_garden_sizing.md` | `resolve/roof_catchment.py`, `resolve/rain_garden.py`, `checks/mep/landscape_drainage.py` — both basins' catchment, prismoidal volume, setbacks and the four leader extensions (`tests/test_catlin_gardens.py`) | live |
