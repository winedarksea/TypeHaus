...
**They grew 7 1/4" on 2026-09-03 and gave it back on 2026-09-05, and the bells never moved
relative to the beam soffits.** The court surface dropped 7 1/4" so heavy rain would pond
outside `D-B-PATIO` rather than cross its threshold, and `_pier_bell_bottom_ft` is measured
from the court — 42" below it, by excavation — so the bells followed the ground down and the
shafts made up the difference. Then the court came back flush and the bells came back with
it. **In between, `_pier_bell_bottom_ft` was PINNED at the dropped elevation for a day** on
the argument that 49 1/4" of cover is more conservative than 42" and that re-opening this
note was not worth 0.1 cy of shaft; the owner's instruction reverses that, and the constant
...
150 pcf = **1,258 lb**. (It was 1,329 lb at the 11.287' height the flood step gave it; 7 1/4"
of 12" round is 71 lb of concrete, and it lands in D on both. The figure is 1,258 again
because the shaft is the length it was before the step, not because anything was rounded
back to it — `_pier_bell_bottom_ft` is derived from the 42" rule and the court is flush.) (`PT-SG-FCOL` was a 20" round until 2026-09-03 — `π×10² = 314.2 in²`
...
```
PT-SG-COL    128.1875 / 12  =  10.68         }  both far past 3.
PT-SG-FCOL   128.1875 / 12  =  10.68         }  both are COLUMNS.
```
...
(k l_u)^2 = 128.1875^2            16,432 in^2       16,432 in^2
P_c = pi^2 EI / (k l_u)^2         765,405 lb        592,910 lb
delta_ns = 1/(1 - P_u/0.75 P_c)     1.019             1.025
```

**A 1.9% magnifier on the back column and 2.5% on the front**, because both are at a few per
cent of capacity. (They were 2.2% and 2.8% while the flood step had these shafts 7 1/4"
longer: `P_c` goes as `1/(k l_u)^2`, so that much shaft is worth 10.5% of the buckling load
either way. It is the one place the court's elevation is felt as a stiffness rather than as
a weight, and it was never near binding in either state.) The split is `E_c`, which goes as `sqrt(f'c)`: a stiffer column magnifies
...
magnified by delta_ns       0.978"              0.984"
...
V_u = q_u x (A_bell - 425.79)
  COL   15.242 x (706.86 - 425.79)  =  15.242 x 281.07  =  4,284 lb
  FCOL  10.583 x (1,017.88 - 425.79) =  10.583 x 592.09 =  6,266 lb
...
phi V_n = 0.60 x 2.66 x 70.711 x 82.54 x 10  =  93,150 lb        (both bells)
  ...
  COL   4,284 / 93,150  =  d/c 0.046   OK
  FCOL  6,266 / 93,150  =  d/c 0.067   OK
```
...
PT-SG-COL   R = 15"
  ...
  M_u  = 15.242 x 197.34 x 4.005                             = 12,046 lb-in
...
PT-SG-FCOL  R = 18"
  ...
  M_u  = 10.583 x 320.29 x 5.268                             = 17,855 lb-in
