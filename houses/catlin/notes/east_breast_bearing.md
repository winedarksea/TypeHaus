...
| north / south | y = 128 1/2" / 79 1/2" | a **doubled** trimmer each side, first ply's axis **on** the edge |
...
**That costs the 4'-0" line: the header now spans 4'-1", not 3'-9 1/2".** IRC R502.10's
threshold for a single header on single trimmers is therefore crossed — and it changes
nothing, because **R502.10 governs sawn lumber and these are I-joists**, the resolver had
already given the opening doubled trimmers and a 2-ply LVL header, and it did not change
that header's section when the span grew. The cut ends need the joist
