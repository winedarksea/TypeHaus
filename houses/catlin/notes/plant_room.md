...
**worst location in the house** for this: two exterior walls, four windows, and a floor and
ceiling that are wood I-joist systems shared with conditioned space above and below. It had
an exterior French door as well until 2026-09-03; see Openings.
...
- `WIN-S-PLANT1/2/3` are retyped to `WT-3048-HP` and `WT-2736-HP`: identical dimensions,
  better glass (U ≈ 0.14), warm-edge spacer, thermally broken frame. This follows the
  established `-T` tempered-twin precedent — *adding a better unit is a retype, never a move*
  — so no facade or framing rule is disturbed. A third type, `WT-3048-HP-T`, carried
  `WIN-S-PLANT2` while that unit sat within 24" of `D-S-DECK-W` and R308.4.2 made its
  location hazardous; with the door gone the location is not hazardous and the type is gone
  with it.
- **Every unit in this room is FIXED, and that is the U-0.14 spec talking, not a preference.**
  A whole-window U-0.14 is only reachable on a fixed sash: an operable one adds a second frame
  profile, a sash-to-frame gap and hardware through the thermal break, and the same glass
  package in a casement lands nearer U-0.18–0.20 — back inside the condensing band this whole
  retype exists to escape. It is also the right answer for the liner. An openable sash is an
  owner-operable hole in a Class I vapour barrier and a way to put the room at outdoor
  humidity with the humidifier still running; the room is ventilated mechanically by design
  (`REG-S-HP-PLANT` in, `REG-S-ERV-PLANT-EXH` out, both dampered), never by opening a window.
  R303.1's openable-area half is therefore carried by Exception 1, which the room passes on
  its grow tubes and its supply terminal (42.5 fc against the 6 fc the exception asks for),
...
- **`D-S-DECK-W` IS DELETED (2026-09-03), and `WIN-S-PLANT4` stands on its centre.** A 60"
  exterior French door was the one hole in the six-sided liner that no detail closes: it is
  mostly frame, and frame and edge-of-glass run 5–8 °F below centre of glass, so its threshold
  condenses at design no matter what unit is bought; every use of it dumps 70 %-RH air onto
  the balcony and pulls −15 °F air across the liner; and it is the only opening in the room
  that cannot be a fixed, gasketed unit. The replacement is another `WT-3048-HP` at the
  door's exact centre (x 14'-8"), which is a stud line on `W-S-S1` and continues `PLANT1`/
  `PLANT2`'s 5'-4" beat, still columning over `WIN-M-BED-S2` below. The balcony is reached
  from the study through `D-S-DECK-E`, the only balcony door the source ever drew.
