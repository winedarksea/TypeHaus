...
**Good news on the existing wall.** The model's `polyiso` is 1.0 perm-in (0.5 perm at 2")
and `eps` 3.9 perm-in, so `CATLIN_EXT_2X6`'s exterior CI stack is ≈ 0.4 perm — Class II,
slow but real outward drying. The wall does not need re-engineering. **But the spec must say
glass-faced or unfaced polyiso, never foil-faced.** Foil-faced polyiso (the house's own
`polyiso-foil`, 0.03 perm) would sandwich the stud bay between two vapour barriers with
wet-prone wood at 25 °F in between — the one genuinely unrecoverable mistake available here,
...
2. **Matched extract**: `REG-S-ERV-PLANT-EXH` (25 cfm) on `DU-S-PLANT-EXH`, a dedicated
   dampered branch riding the `FS-ATTIC` bay at y=4'-8" and dropping through `SF-S-DUCT` to
   `EQ-B-ERV`'s stale side. Ceiling-mounted, at the far end of the room from the supply,
   because humid air stratifies and the wettest air in the room is the air against the
   I-joists.
