...
| inner girt | **SPF** 2x4 flat, horizontal, 24" o.c. | on the block-1s, 1-1/2" → 3", buried in foam |
| block-2 | **KDAT** 2x4 offcut, same piece | on the inner girt, MID-BAY (8" off block-1), 3" → 4-1/2" |
| outer girt | **KDAT** 2x4 flat, same 24" courses at the SAME elevations | on the block-2s, 4-1/2" → 6"; the cladding nailer and the window mount plane |
...
**The blocks are on the STUD module, the girts on their own.** A girt course climbs a 24"
elevation module measured from the wall base; the blocks under it are what carry it back to
...
between two adjacent nailer courses anywhere beside an opening — above a head or below a sill
— is **23"**, against the field's own 20-1/2" clear (24" o.c. less the 3-1/2" board). The
median is 18-1/2". The girt field around every opening in the house is as regular as the field
away from it. Any window taller than 24" necessarily interrupts some courses; that is inherent
to a horizontal girt and is not a defect, because the courses it interrupts are replaced by the
opening's own head and sill courses at the same job.

**15 of 44 openings already land a head or a sill exactly on a course line**, which is the
best case: the opening's head or sill course *is* the field course and nothing extra is cut.
Every second-storey and attic opening whose sill is a whole number of feet above its wall base
does this for free.

**14 openings leave a sliver** — a field course ending 0.4" to 2.5" short of the opening's own
head or sill course, i.e. two nailers within a board's width of each other, one of them
redundant. They are:

| sliver | openings |
|---|---|
| 0.4" below the sill | WIN-M-KITCH, WIN-M-KITCH-N, WIN-M-KIT-E |
| 1.0" below the sill | WIN-S-PLANT1/2, WIN-S-STUDY1/2, WIN-A-S-JUL-E/W, WIN-A-S2, WIN-A-S3 |
| 1.1" above the head | WIN-M-KITCH, WIN-M-EAST-MID |
| 2.5" above the head | WIN-S-BED1, WIN-S-BED2 |

**Why they are not being nudged away.** The obvious fix is to move each window down (or up) by
its own sliver so the two courses abut. For the second-storey and attic group that is a clean
1" and it would work. For the main-storey group it is **7/16"**, and 7/16" is not a coincidence
— it is the offset between the main storey's finished floor and its wall base. The sills are
authored at round heights *above the floor*; the courses run at round heights *above the wall
base*. Chasing the sliver would trade a clean authored dimension for a clean framing
coincidence, on a facade whose composition is documented and deliberate
(`houses/catlin/CLAUDE.md`, Facade rules — 20 windows were moved once already to put the whole
house on one horizontal module, and that grid is not free to disturb).

If the owner ever does want them gone, **the better lever is the course origin, not the
windows.** `furring.course_elevations` counts from `rw.z0_m`, the wall base. Phasing it off the
storey's finished floor instead would put the whole main storey on the sills' own datum and
close all four main-storey slivers at once, without moving anything. That is a one-function
change and it is deliberately not being made today: it would move every course in the house by
7/16" to fix a 7/16" problem in four of them.

**What a designer should do with a NEW opening**, stated once so it does not have to be
re-derived: land the **head** on a course line, or the **sill** 3-1/2" above one. Either makes
that opening's head or sill course the field course. `haus build` then shows it, and nothing
about the rule needs enforcing — a miss costs a redundant board, not a defect.
...
reads **R-40.7**; the honest number is **≈ R-37.5**, and the difference is two modelling
...
- **A simpler fastener story.** One screw per block instead of two, and every screw
  wood-to-wood with continuous lateral support. 4,154 five-inch SDWS against the Swinburne's
  2,570 four-inch ones — the count is up because a girt wall blocks every course at every stud
  station (16" × 24") where an outrigger wall blocked every outrigger every 40", and one screw
  instead of two takes half of that back.
