...
- **24" o.c. is deliberate, and there is no 16" zone at the overhead door.** W-G-E is nonbearing — the ridge runs E–W and the trusses bear on W-G-S/W-G-N — and the 16'-0" opening is carried by its own 2-ply 14" LVL on jamb packs sized from the opening, not from field spacing. Field studs beside a nonbearing opening carry nothing extra. The garage's window pair and its service door were re-stationed onto the 24" grid the same day; D-G-OVERHEAD stays where it is and its off-module advisory is a recorded decision (`preferences.toml [checks] suppress`), because every legal station moves the ICF grade beam under it. **That suppression's ORIGINAL reason — a visibly asymmetric 5'-0"/3'-0" pair of wainscot piers — died with the wainscot on 2026-09-03**; the base band is uniform now and does not care where the door sits. What is left is the grade-beam chain, which is a cost of moving rather than a reason not to. Open question, not a settled one.

- **THERE IS NO WAINSCOT SINCE 2026-09-03. The 24" stem band is the whole base skin, on all four walls.** From a hemmed drip **2" below grade** to the stem top at **22" above** it, PVDF-painted aluminium flat sheet on a 1/4" vented standoff, fixed with 316 stainless gasketed screws into the ICF webs, every edge hemmed. It is the outside half of this note's own both-faces requirement for the exposed EPS, and it now stands alone rather than behind something taller.

  **What was deleted, and why the piers lost nothing.** A 4'-0" wainscot stood on the two 4'-0" strips of east wall flanking the overhead door and wrapped 4'-0" around each of the SE/NE corners — Glen-Gery Columbia Roman Maximus soldier brick until 2026-09-02, then PVDF aluminium sheet on a 1-1/2" cavity, capped at a round 4'-0". **This band always ran behind it**, because that wainscot was a vented, drained rainscreen open at its base and the foam behind it needed the same continuous protection as the foam beside it. So deleting the wainscot took a wear layer off two wall strips and took nothing off their protection, and the east elevation now reads as one uniform base course instead of a tall panel over a short one. Gone with it: the wainscot's cap flashing, its KDAT furring and cleats, and — from the brick before it — the mid-stack ICF brick-ledge form, the 20" -> 24" footing widening on four stem segments, through-wall flashing and weeps at the base and under the cap, corrugated ties in three bed joints at a 16" o.c. spacing the wall's 24" studs could not give, and the ~8 lf per pier of flat 2x6 tie blocking bought to reach it.

  **GAUGE IS THE DENT LEVER, AND IT SURVIVED THE SHRINK.** There is no backer: the sheet spans its fixings and never touches the foam, so dent resistance is gauge and fixing spacing, not a board behind it. **0.040" minimum, 0.050" preferred.** With only a ~24" band left, 24" trim coil is the obvious buy and it is the wrong one — stock 48" x 120" architectural sheet rips into exactly **two 24" bands with no waste**, so the heavier metal costs nothing per SF over coil, and this is still the zone a shovel corner and a plow work in. **Second best, only if a supplier cannot get sheet: 0.024" heavy-gauge 24" trim coil** — the thickest the trim-coil product line reaches. **Never 0.019" standard trim coil**, which takes a permanent dimple.

  **THE ONE NEW RULE IS A CORROSION RULE.** The corrugated panel above is 26 ga PVDF-coated **steel** and this band is **aluminium**. The Z-flash between them must be aluminium, and the two must never lap metal-to-metal — sealant or EPDM between, the Z's upper leg behind the corrugated. In a salted splash zone that contact line is where the detail fails, and nothing in the engine grades dissimilar metals. Aluminium must also never touch concrete or fresh mortar: alkali strips its oxide film. On site the corrugated terminates at the Z with its vented base closure there; the model bills the panel full height behind the band.
...
- Provide Z-flashing / drip edge at base of exterior wall (liquid flashing recommended) to direct water out over the foundation / protective coating. **Aluminium, not steel** — it lands on the aluminium stem band, and see the corrosion rule above on why that contact matters here. **This is built as of 2026-09-03**: `STEM_TOP_Z_FLASHING` in `plan/storeys/garage.py` is six runs at the stem top, broken at the overhead and service door gaps where there is no stem to band.
