...
import {
  formatFtIn, M_PER_FT, openingHostWall, snapWorld, orthoLock,
} from "../model/geometry";
import { SunIndicator } from "./SunIndicator";
import { ZoomControls } from "./ZoomControls";
import { BackgroundGrid } from "./plan/PlanChrome";
import { CanvasHud } from "./plan/CanvasHud";
import { CanvasObjectFootprint, ClearanceOverlays } from "./plan/ObjectShapes";
import { WallDimension, WallShape } from "./plan/WallShapes";
import { WallNodeHandles } from "./plan/WallNodeHandles";
...
import { useCanvasCommits } from "./plan/useCanvasCommits";
...
export function Canvas2D() {
  ...
  const pendingTransforms = useStore((s) => s.pendingTransforms);
  ...
  return (
    <>
      <svg
        ...
        onPointerLeave={() => { if (tool === "placeable") setCursor(null); }}
        ...
        <PlanLabelLayer>
          ...
          {footprintObjects
            .map((item) => <CanvasObjectFootprint key={item.uid} item={item}
              ...
              pending={pendingTransforms[item.uid]}
              onDrop={dropPlaceable} onRotate={rotatePlaceableFromHandle} />)}
          ...
          {tool === "select" && selection.kind === "wall" && (() => {
            ...
            return w ? <WallNodeHandles wall={w} model={model} activeStorey={activeStorey}
              nodeDrag={nodeDrag} setNodeDrag={setNodeDrag} setPreviewGeom={setPreviewGeom}
              storeyNodes={storeyNodes} snapNodes={snapNodes} tolM={tolM} gridM={gridM}
              project={project} unproject={unproject} nearestNodeTag={nearestNodeTag} /> : null;
          })()}
          {tool === "placeable" && cursor && placementType && canvasTypes.get(placementType) &&
            <PlaceableGhost type={canvasTypes.get(placementType)!} cursor={cursor} rotation={placementRotation}
      ...
      <CanvasOverlays
        ...
        dimWall={dimWall} setDimWall={setDimWall} onCommitDim={(m) => { setDimWall(null); void commitDim(dimWall, m); }}
