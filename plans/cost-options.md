...
| **Sunken garden / porch / balcony** — the freestanding concrete structure | **$45,200–82,100** | 4.9% / 4.3% | −$2,700–4,800 on 2026-09-10: the court went 28' → 26' and all five strips 8'-0" → 7'-0". Balcony alone is $10,300–19,100 of it; brick veneer's BOM row is $2,455–3,876 since the 2026-09-04 buff swap (its ablation figure is stale — see *Cost cutting*) |
...
| ⚠ **Shorten the sunken court further, 26' → 23'-4" N–S** | **$1,900–3,500** | arithmetic, 2026-09-10 | no | garden-footing engineering; the boom pump | **2'-8" is ALL that is left. There is a hard structural floor at 23'-4".** Shortening removes base friction from the capacity and NOTHING from the demand — the E–W thrusts cancel identically, so the resultant is the south wall's thrust over the court's WIDTH, which does not change. At 23'-4" the system reaches the 1.50 IRC R404.4 minimum and stops. This row read "28' → 16' for $9,700–16,600" until 2026-09-10; **that was never available** — 16' fails sliding in all four soil corners. The first 2'-0" has been taken (see below) |
| ✅ **TAKEN 2026-09-10 — the court went 28' → 26' and the strips 8'-0" → 7'-0"** | **$2,700–4,800** | arithmetic | no | the two rows above | Sliding FS 1.80 → 1.63 (20% over the code minimum → 8.6%), and the μ = 0.25 no-stone sensitivity 1.29 → 1.16. Both were already stated exposures. **The garden got BIGGER**: the planted field 147 → 160 sf, because the field is bounded by the footing toe reach and that fell 4'-6" → 3'-6" a side. Rebar 4,828 → 3,515 lb, which is real in the yard and invisible in the estimate while `[rebar_inclusive]` holds |
...
| **Engineer the garden footing base and stone bed** | $3,200–8,000 | arithmetic | no | court-shortening | Nothing, if it stamps. **Now the highest-value item in this table per dollar.** The whole sliding margin rides on μ = 0.35 from the washed-stone bed against 0.25 on the native GM, and a boring that supported 0.40 on the native soil would hand back four times what the 2026-09-10 shortening spent |
...
| **Boom pump, several days** | inside `concrete-pumping` $4,600–7,800 — probably short | Forced by the sunken garden: 27.1 cy of 10' wall at the bottom of a 9' court, 5" from the house (25.95 of `SUNKEN_GARDEN_WALL` plus the 1.10 veneer beam; it was 29.2 before the court shortened on 2026-09-10). A chute cannot reach it |
...
| **Rebar, ~5 tons** | $10,000–18,000 | Still inside the `[concrete]` and `[wall_structure]` $/cy rates, and **the "nothing enforces that" is fixed as of 2026-09-03**: `[rebar_inclusive]` in `prices.toml` declares which rates still contain their steel, and pricing `[reinforcement]` against one that does is a hard error naming both sides (`cli/price_file._check_rebar_not_double_billed`). `takeoff/reinforcement.py` bills the tonnage NOW, priced at nothing, so the quantity can be read against this row before any rate is cut — 1.76 tons of it so far, from the sunken-garden court alone. **Read that against 2.41 tons on 2026-09-09, not as growth**: the court's footing mat came down `#6 @ 10"` → `#5 @ 12"` when the strips narrowed, which removes steel and no concrete, so the house's lb/cy ratio fell 31.6 → 23.8. The rest of the house is not yet migrated to `ReinforcementSpec`, and that unbilled steel — not this cut — is still the gap to ~5 tons |
| Excavation has no quantity | the weakest number in the file | $24,000–55,000 as a lump, so no design change can ever demonstrate a saving against it. The sunken garden alone implies 185–235 cy of extra cut (it was 200–250 before the court shortened 28' → 26' on 2026-09-10; the excavation loses the court's own 38 sf at ~9' plus the footings' outboard over-dig) |
