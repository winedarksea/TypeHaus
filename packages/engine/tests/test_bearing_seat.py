...
def test_the_seat_is_the_number_the_basement_walls_are_authored_to(catlin_model):
    ...
    # It was 14 for one afternoon on 2026-09-05, while W-B-S1 was split at x=4'-8" for the
    # rotated sauna's south face; the shrink that followed put the room wholly on the garden
    # curb and W-B-S1 back together, so it is 13 again. The two new walls that replan DID
    # add (W-B-WELL, W-B-CL-N) are framed partitions on the slab, not pours, and never
    # entered this set.
    assert len(walls) == 13
