...
def test_floor_joist_counts_match_old_model(catlin_model):
    ...
    # Less two on the west: FO-S-ERV-CHASE's trimmer pack and header (2026-09-24). Less
    # three since 2026-09-25: the y=25'-4" line is FO-S-STAIR's full-span south pack.
    assert len(west_joists) == expected_positions - 3
...
def test_catlin_is_all_electric_with_no_gas_appliance(catlin_model):
    ...
    # mixing box where the ERV leg joins System 1's return. The extract side is deliberately
    # absent: EQ-T-ERV-MANIFOLD-6-EXH is the same part as the supply manifold and the same
...
# SAUNA_LINER_ON_BASEMENT_8 joined on 2026-09-05 and left the same day: the rotation put
...
def test_garage_wood_framing_uses_its_structure_layer_centerline(catlin_model):
    ...
    for wall in catlin_model.walls:
        if not wall.tag.startswith("W-GF-"):
            continue
        # rstrip digits, not just "12": W-GF-S3/N2 (the SE/NE wainscot-return corner pieces)
    ...
    # Excludes "W-G-WAIN-*": the wainscot (and its SE/NE corner returns) also start with
    # "W-G-" but are a freestanding skin, not this wood-framed wall system. It was
    # "W-G-BRICK-*" until 2026-09-02, when the veneer became aluminium sheet — a rename this
    # filter has to track, and the failure mode if it does not is this test quietly grading
    # four extra walls rather than erroring.
    garage_walls = [wall for wall in catlin_model.walls
                    if wall.tag.startswith("W-G-") and "WAIN" not in wall.tag]
...
def test_upper_storey_studs_stand_over_studs(catlin_model):
    ...
    #   * **124/255 later the same day**, and it is the same two walls moving one each. The
    #     under-stair replan slid N-B-BA-SE 1 5/16" west onto the stair well's partition
    #     line, which is W-B-CW2B's start node: `W-M-CLN2` 7 -> 8 for the same reason it went
