...
def test_each_system_owns_exactly_its_own_temperature(catlin_model_ro, catlin_ifc):
...
def test_every_authored_pipe_run_lands_in_exactly_one_system(catlin_model_ro, catlin_ifc):
    ...
    run_tags = {run.tag for run in catlin_model_ro.pipe_runs}
...
def test_every_accessory_is_a_typed_device_and_never_a_footing(catlin_model_ro, catlin_ifc):
    """The ``_SOLID_IFC_CLASS`` fallback wart: a category with no row becomes IfcFooting."""
    assert catlin_model_ro.pipe_accessories, \
    ...
    for accessory in catlin_model_ro.pipe_accessories:
...
def test_each_accessory_kind_gets_the_right_valve_type(catlin_model_ro, catlin_ifc):
...
def test_a_run_carries_its_material_finish_and_insulation_into_the_file(catlin_model_ro,
                                                                       catlin_ifc):
    insulated = next((r for r in catlin_model_ro.pipe_runs if r.insulation), None)
...
def test_a_routed_run_exports_once_and_never_as_a_footing(catlin_model_ro, catlin_ifc):
    ...
    run_solids = [s for s in catlin_model_ro.solids
