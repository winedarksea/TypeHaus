...
def _numbers(sheets):
    """The numbers of a composed index, in order.

    Takes the index rather than the model: composing one is the expensive thing this
    module does, and eight tests were each composing their own (→ ``catlin_sheet_index``).
    """
    return [sheet.number for sheet in sheets]


def test_every_sheet_number_is_well_formed(catlin_sheet_index):
    bad = [n for n in _numbers(catlin_sheet_index()) if not _NUMBER.match(n)]
    assert not bad, f"not NCS sheet numbers: {bad}"


def test_the_type_digit_agrees_with_the_content(catlin_sheet_index):
    ...
    numbers = set(_numbers(catlin_sheet_index()))
...
def test_details_are_the_five_series_not_the_four(catlin_sheet_index):
    """Type 4 is a large-scale VIEW — an enlarged plan. A junction cut is type 5."""
    numbers = _numbers(catlin_sheet_index())
...
def test_elevations_come_before_sections(catlin_sheet_index):
    numbers = _numbers(catlin_sheet_index())
    assert numbers.index("A-201") < numbers.index("A-301")


def test_disciplines_are_emitted_in_order(catlin_sheet_index):
    """G, C, S, A, P, M, E — each discipline contiguous and in NCS order."""
    seen: list[str] = []
    for number in _numbers(catlin_sheet_index()):
...
def test_the_three_schedules_are_three_sheets(catlin_sheet_index):
    """Doors, windows and room finishes have three readers and three sheets."""
    titles = {s.number: s.title for s in catlin_sheet_index()}
    assert titles["A-601"] == "Door schedule"
    assert titles["A-602"] == "Window schedule"
    assert titles["A-603"] == "Room finish schedule"


def test_the_symbols_legend_explains_what_the_set_draws(catlin_sheet_index):
...
def test_no_sheet_number_is_used_twice(catlin_sheet_index):
    numbers = _numbers(catlin_sheet_index())
    assert len(numbers) == len(set(numbers))


def test_s002_is_gone_and_s001_replaces_it(catlin_sheet_index):
...
def test_sheet_numbers_ascend_within_a_discipline(catlin_sheet_index):
    ...
    for letter in _DISCIPLINE_ORDER:
        series = [n for n in _numbers(catlin_sheet_index()) if n.startswith(f"{letter}-")]
