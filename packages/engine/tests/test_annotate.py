...
def test_place_column_is_a_uniform_ladder_for_single_line_labels():
    ...
    placed = place_column(entries, x=-5.0, z_top=100.0, step=2.6, height=1.6)
...
def test_place_column_grows_rows_to_fit_wrapped_text():
    ...
    placed = place_column(entries, x=0.0, z_top=50.0, step=2.6, height=1.6)
...
def test_dodge_pushes_overlapping_boxes_down_and_is_deterministic():
    a = place_column([LabelSpec(text="aaaa")], x=0.0, z_top=10.0, step=2.6)[0]
    b = place_column([LabelSpec(text="bbbb")], x=1.0, z_top=10.5, step=2.6)[0]
...
def test_dodge_ignores_horizontally_separated_boxes():
    a = place_column([LabelSpec(text="left")], x=0.0, z_top=10.0, step=2.6)[0]
    b = place_column([LabelSpec(text="right")], x=100.0, z_top=10.0, step=2.6)[0]
    out = dodge([a, b])
    assert out == [a, b], "no overlap in u → nothing moves"


def test_dodge_respects_fixed_obstacles():
    lab = place_column([LabelSpec(text="mmmm")], x=0.0, z_top=10.0, step=2.6)[0]
...
def test_leader_box_matches_label_box_alignment():
    ...
    node = Leader(anchor=NamedPoint(xy=(50.0, 5.0)), at=(10.0, 5.0), to=(50.0, 5.0),
                  text="abc")
    assert leader_box(node) == label_box((10.0, 5.0), "abc", 1.6, "right")
    # text right of its target grows rightward
    node = Leader(anchor=NamedPoint(xy=(0.0, 5.0)), at=(10.0, 5.0), to=(0.0, 5.0),
                  text="abc")
    assert leader_box(node) == label_box((10.0, 5.0), "abc", 1.6, "left")
