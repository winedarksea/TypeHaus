...
from typehaus.cli.takeoff_report import (
    _print_basis,
    _print_bid_ladder,
    _print_driver_overlaps,
    _print_per_sf,
    _print_run_schedule,
    _print_takeoff_summary,
    _unpriced_note,
)
...
def takeoff(
    ...
    strict: bool = typer.Option(False, "--strict",
                                help="exit 1 when any BOM row group has no prices.toml row"),
) -> None:
    ...
    """
    payload = _takeoff(house, as_json, summary, csv, runs)
    unpriced = (payload.get("cost_estimate") or {}).get("unpriced") or []
    if strict and unpriced:
        console.print(f"[red]--strict: {len(unpriced)} unpriced row group(s)[/red]")
        raise typer.Exit(1)


def _takeoff(house: Path | None, as_json: bool, summary: bool, csv: Path | None,
             runs: bool) -> dict:
    import json
    ...
    payload = {"framing": dict(sorted(framing.items())),
               ...
               "freeze_protection": bom["freeze_protection"],
               "ducts": bom["ducts"],
