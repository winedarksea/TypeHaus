...
def _propose(model, targets: list[str], *, mode: str, slope: float | None,
    ...
    for target in targets:
        origin, root, diameter_m, serves, storey, system = _endpoints(
        ...
        graph = build_graph(space, terminals)
        ...
        if system == "drain" and not budget.feasible:
            ...
            continue
        if system == "drain":
            points = pipe_trade.elevate(points, budget, grade_in_per_ft=grade)
            notes.append(f'gravity: {grade:.3f}"/ft over {developed_ft:.2f} ft, '
                         f'{budget.slack_in:.2f}" of head to spare')
...
def _endpoints(model, target: str, mode: str, problems: list[str]):
    """``(origin, root, diameter, serves, storey, system)`` for one target, or Nones.
    ...
    if mode in ("run", "tree") and target in runs:
        ...
        root = _discharge(model, run, problems)
        if root is None:
            return (None,) * 6
    ...
    if mode in ("fixture", "unconnected"):
        ...
        floor_m = next((s.elevation.meters for s in model.plan.storeys
                        if s.tag == storey), 0.0)
        root = _nearest_main(model, point, floor_m, problems, target)
        if root is None:
            return (None,) * 6
        del element
...
def _discharge(model, run, problems: list[str]):
    ...
    return (other.path[index][0], other.path[index][1], other.z_m[index])
