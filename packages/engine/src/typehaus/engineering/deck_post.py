...
#: ACI 318 defines a PEDESTAL as a member whose unsupported height is not more than three
#: times its average least lateral dimension. Plain concrete pedestals are permitted; plain
#: concrete COLUMNS are not. The ratio is therefore not trivia — it is the whole question of
#: which chapter applies, and both catlin piers are well past it (10.7 and 6.4).
...
def _one(pier: _Pier) -> EngineeringRecord:
    ...
    notes = (
        ...
        f"Height / least lateral dimension = {ratio:.1f}. ACI 318 calls a member a PEDESTAL "
        f"at {PEDESTAL_HEIGHT_RATIO:.0f} or less and permits it to be plain concrete; past "
        f"that it is a COLUMN, and a column may not be plain.",
        ...
        f"(1% of gross), plus ties. Sizing it is the engineer's, and this engine has nowhere "
        f"to record the answer.",
    ...
    return EngineeringRecord(
        ...
        missing=(f"the vertical reinforcement in {pier.tag} — ACI 318 does not permit a "
                 f"plain concrete COLUMN at any stress, and `Post` carries no field in "
