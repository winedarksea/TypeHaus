...
def ess_detection(ctx: CheckContext) -> list[Finding]:
    """R327.7 — smoke and heat detection *in* the room the ESS is in.
    ...
    a location — the failure mode that matters announces itself as heat long before there
    is smoke outside the cabinet — so this check wants one of each.

    In the room, with no proximity allowance. This deliberately does *not* reuse the
    buffered-neighbour pattern ``alarms.py::garage_heat_and_co_alarms`` uses for the
    garage's CO coverage, and the difference is the point: that rule is about protecting
    the people next door, so a head in the adjacent room is the right place for it. This
    one is about sensing the cabinet. A head on the other side of the enclosure senses the
    room the enclosure is there to protect, which is a different measurement wearing the
    same name — and a 3'x4' ceiling holds two heads without difficulty.
    """
    ...
    for room_tag in sorted({getattr(b, "room", None) for b in batteries} - {None}):
        covering = [a for a in alarms if a.room == room_tag]
        for kind, label in ((AlarmKind.SMOKE, "smoke"), (AlarmKind.HEAT, "heat")):
            ...
            if found is None:
                out.append(_fail(cid, f"ESS room {room_tag} has no {label} alarm in it; "
                                      "R327.7 requires both smoke and heat detection at "
                                      "the storage system",
                                 (str(room_tag),), code))
            else:
