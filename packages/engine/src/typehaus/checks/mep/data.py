...
# Every red here is ``advisory_fail`` (WARN + FAIL): advisory, never a permit blocker.
...
def data_reachability(ctx: CheckContext) -> list[Finding]:
    ...
    if not runs:
        return [advisory_fail(cid, f"{len(devices)} data device(s) but no data raceway carries "
    ...
    for device in sorted(devices, key=lambda d: d.tag):
        if device.tag in served:
            ...
            out.append(advisory_fail(cid, f"data device {device.tag} is not the to_ref of any data "
                                          "raceway — it has no pathway back to the head end",
                                     (device.tag,)))

    if len(heads) > 1:
        out.append(advisory_fail(
            cid, "more than one head end — these data devices are served by no "
    ...
    for run in sorted(remaining, key=lambda r: r.tag):
        out.append(advisory_fail(
            cid, f"data raceway {run.tag} starts at {run.from_ref or '(nothing)'}, "
