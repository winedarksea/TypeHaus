...
The CONCRETE half moved to ``resolve/mep_concrete.py`` on 2026-09-19 (this file was 622
lines, over AGENTS.md's 500). ``concrete_bands``, ``concrete_crossings`` and
``_CONCRETE_SOLID_CATEGORIES`` are re-exported below so ``routing/obstacles.py``,
``resolve/mep_sleeves.py``, ``resolve/mep.py`` and ``tests/test_concrete_bands.py`` keep
the spelling they were written with. New callers should import from ``mep_concrete``.
"""
...
from typehaus.resolve.mep_concrete import (  # noqa: F401  (re-exported: see the note above)
