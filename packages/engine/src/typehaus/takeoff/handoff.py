...
def pe_readme(*, house: str, generated: str, engine_version: str, content_hash: str,
    ...
    out += [
        ...
        "",
        "## Load it into your own software",
        "",
        "`model.ifc` also carries the **IFC4 structural analysis view** — the engineered items",
        "and their load path as analytical members, supports with boundary conditions, and load",
        "cases — which SAP2000 and ETABS import directly (choose the structural view, not the",
        "coordination view). `analysis/` holds the same model three more ways: `centreline.dxf`",
        "for RISA-3D, `members.csv` for ForteWEB / WoodWorks Sizer / Enercalc, and",
        "`model.pynite.py`, a script that solves it in the open-source PyNite solver.",
        "`analysis/README.md` says which to open, and states every fixity, release and load",
        "the model claims, each with its basis. Those claims are this engine's; you are",
        "entitled to reject any of them, and the README tells you where each came from.",
        "",
        "## Integrity",
        "",
