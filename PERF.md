...
`resolve.rebar` lays out ~1,190 bar pieces (2026-09-17, min of 8 runs of the stage on a quiet
machine, not the 20-rebuild median above). Budgeted at 150 ms in the perf guard. Most of it
is `junctions.py` keeping corner and splice bars from overlapping (bounding boxes first, then
segment distance) and the shapely mat clipping.
